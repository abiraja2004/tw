...
class FBPost(object):
    ...
    def __repr__(self):
        return ("<FBPost (%s-%s) %s: %s>" % (self.getLanguage(), self.getTwitterLanguage(), self.getUsername(), self.getText()))

    @classmethod
    def createFromMongoDoc(cls, doc):
        res = GnipActivityFBPost(doc)
        ...
        if not 'x_feed_type' in res.d:
            res.d['x_feed_type'] = 'facebook'            
        return res

    @classmethod
    def createFromRawGnipFBPost(cls, activity):
        res = GnipActivityFBPost(activity)
        res.normalize()
        return res
...
class GnipActivityFBPost(FBPost):
    
    def __init__(self, activity):
        FBPost.__init__(self)
        self.d = activity
    ...
    def getUsername(self):
        return self.d['author:name']

    def getText(self):
        return self.d['activity:content']
