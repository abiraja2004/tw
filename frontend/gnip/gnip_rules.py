...
class GnipTwitterRules(object):
    def __init__(self, accountname, UN, PWD):
        self.base64string = base64.encodestring('%s:%s' % (UN, PWD)).replace('\n', '')
        self.url = "https://api.gnip.com/accounts/%s/publishers/twitter/streams/track/prod/rules.json" % accountname
        self.initLocalRules()
...
class GnipCollectionRules(object):
    def __init__(self, accountname, UN, PWD):
        self.base64string = base64.encodestring('%s:%s' % (UN, PWD)).replace('\n', '')
        self.url = "https://%s.gnip.com/data_collectors/1/rules.json" % accountname
        self.initLocalRules()
        self.clean = False  # indicates sync between server 
        self.listGnipRules()
...
if __name__ == '__main__':
    Url = 'https://api.gnip.com/accounts/promored/publishers/twitter/streams/track/prod/rules.json'
    ...
    #print r.getRulesLike("2")
    #print r.getRulesLike("tag", match_tag=True)